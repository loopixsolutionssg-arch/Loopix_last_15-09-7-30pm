      ...
      <Container maxWidth="lg">
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={4}>
            <Paper elevation={0} className="glass-card" sx={{ p: 2, height: { md: 282 }, overflow: { xs: 'visible', md: 'hidden' } }}>
              <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 1 }}>
                Projects
              </Typography>
              <List sx={{ overflowY: { md: 'auto' }, maxHeight: { md: 480 }, display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.5 }}>
                {projects.map((p) => (
                    ...
                    <Grid container spacing={1}>
                      {projects.filter((p) => p.id !== selected.id).map((p) => (
                        <Grid item xs={6} key={`rel-${p.id}`}>
                          <Paper elevation={0} className="glass-card" sx={{ p: 1, cursor: 'pointer' }} onClick={() => navigate(`/projects/${p.id}`)}>
                  ...
                  <Grid container spacing={2}>
                    {projects.map((p) => (
                      <Grid item xs={12} sm={6} md={4} key={`more-${p.id}`}>
                        <Paper elevation={0} className="glass-card" sx={{ p: 2, cursor: 'pointer' }} onClick={() => navigate(`/projects/${p.id}`)}>
